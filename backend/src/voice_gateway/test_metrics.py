...
def test_request_latency_histogram_defined_with_labels() -> None:
    ...
    m.request_latency.labels(
        endpoint="/api/voice/turns", status="success").observe(0.42)
    m.request_latency.labels(
        endpoint="/api/voice/turns", status="stt_failed").observe(0.07)

    text = _scrape_text(m)
    assert 'request_latency_seconds_count{endpoint="/api/voice/turns",' \
           'status="success"} 1.0' in text
    assert 'request_latency_seconds_bucket{endpoint="/api/voice/turns",' \
...
def test_request_latency_uses_request_buckets() -> None:
    ...
    m.request_latency.labels(endpoint="/api/voice/turns",
    ...
    for boundary in metrics_mod._REQUEST_BUCKETS:
        ...
        line = 'request_latency_seconds_bucket{endpoint="/api/voice/turns",' \
