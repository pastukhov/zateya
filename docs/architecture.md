# Архитектура

Затея состоит из диктофона StickS3, Voice Gateway и локального Codex Agent. Устройство записывает и воспроизводит звук. Сервер распознаёт речь, оформляет мысли в Obsidian и синтезирует ответ.

![Путь записи от устройства до Obsidian](assets/system-overview.svg)

## Компоненты

```mermaid
flowchart LR
    D["StickS3<br/>запись и воспроизведение"]
    G["Voice Gateway<br/>очередь · STT · TTS"]
    C["Codex Agent<br/>оформление идей и wiki"]
    O[("Obsidian<br/>sources · ideas · wiki · builds")]
    A[("Архив и SQLite<br/>записи · задания")]
    D -->|"PCM · Wi-Fi"| G
    G -->|"текст и контекст"| C
    C -->|"предложение изменений"| G
    G -->|"проверенные Markdown-файлы"| O
    G -->|"WAV"| D
    G --> A
```

Codex Agent работает на хосте и принимает запросы шлюза только через loopback. Учётные данные Codex не передаются в контейнер и прошивку. Токен устройства и внутренний токен агента — разные секреты. Для текущего сценария шлюз использует `VOICE_AGENT_PROVIDER=codex`.

## Голосовой запрос

```mermaid
sequenceDiagram
    participant D as StickS3
    participant G as Voice Gateway
    participant Q as Очередь заданий
    participant S as Распознавание речи
    participant C as Codex Agent
    participant O as Obsidian
    participant T as Синтез речи
    D->>G: запись PCM + ID запроса + токен устройства
    G->>Q: сохранить запись и поставить задание
    G-->>D: ID задания
    Q->>S: распознать запись
    S-->>Q: текст
    Q->>C: текст и контекст базы знаний
    C-->>Q: ответ и предложение изменений
    Q->>O: проверить и сохранить заметки и wiki
    Q->>T: озвучить подтверждённый ответ
    T-->>Q: WAV
    D->>G: опросить состояние и скачать звук
    G-->>D: WAV
```

Устройство создаёт UUID перед отправкой записи. При потере ответа оно проверяет этот же запрос, чтобы не создать дубль. Gateway хранит задания в SQLite; ожидающие задания переживают перезапуск, прерванная обработка автоматически не повторяется. Каждое устройство авторизуется своей парой `X-Device-Id` и bearer-токен. Точные HTTP-пути указаны в [протоколе](protocol.md); выбор режима пользователю не требуется.

## Устройство и настройка

StickS3 записывает моно PCM S16LE 16 кГц по нажатию кнопки и воспроизводит WAV через ES8311. Экран показывает состояние записи, обработки, ответа и ошибки. Setup AP называется `Zateya-Setup-XX`; после подключения к Wi-Fi настройки доступны и через локальную сеть по `zateya-<MAC>.local`. [Подключение Wi-Fi](wifi-profiles-sticks3.md) и [прошивка](flash-sticks3.md) описаны отдельно.

## Заметки и хранение

Сырой текст попадает в `Hermes/sources/` Obsidian vault, оформленные идеи — в `ideas/`, связанные страницы — в `wiki/`, планы и задания — в `builds/`. Подтверждение озвучивается после записи файлов. Затем отдельный процесс коммитит и отправляет изменения в `origin`; подробности в [руководстве по базе знаний](voice-knowledge.md).

Записи и служебные результаты хранятся под `ARCHIVE_ROOT/YYYY/MM/DD/<turn-id>/`; очередь — в SQLite. `/health/live` проверяет процесс, `/health/ready` — конфигурацию и архив, `/metrics` отдаёт метрики без текста записей. Gateway предназначен для доверенной локальной сети или VPN; setup AP открыт для настройки.
