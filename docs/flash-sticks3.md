...
1. При первом старте подключитесь телефоном или компьютером к `Zateya-Setup-XX`. `XX` — последний байт Wi-Fi MAC в нижнем регистре hex.
...
Состояния отображаются на LCD: `ГОТОВА`, `СЛУШАЮ`, `ДУМАЮ`, `ОТВЕЧАЮ`, `ОШИБКА`. Экран ошибки остаётся до нажатия кнопки. Отдельная LED-индикация не используется.
