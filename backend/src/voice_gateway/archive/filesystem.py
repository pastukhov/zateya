...
--------------------------------------------------
The WAV-conversion card (t_cd836888) will make ``_finalize`` delete
``input.pcm`` after a successful conversion (per the base module doc-
string). Until then — and because the M2 ingest contract (test_app_stream
``test_turn_success``) asserts ``input.pcm`` still exists, byte-identical,
after a 200 turn — this implementation KEEPS the raw file on every path.
Deleting it is t_cd836888's change, together with the test updates.
"""
...
class FilesystemArchiveStore(ArchiveStore):
    ...
    def _finalize(self) -> Path:
        """Flush the raw stream and wrap it into ``input.wav`` (ТЗ §17.3).
        ...
        The raw PCM file is KEPT after finalization until t_cd836888
        strengthens this hook (see module docstring).
        """
