...
class GitSync:
    ...
    def _git(self, *args, check=True):
        ...
        env = {key: os.environ[key] for key in ('PATH', 'HOME', 'LANG', 'LC_ALL', 'SSH_AUTH_SOCK')
               if os.environ.get(key)}
        env.update({
            'GIT_TERMINAL_PROMPT': '0',
            'GIT_AUTHOR_NAME': os.environ.get('GIT_AUTHOR_NAME', 'Затея'),
            'GIT_AUTHOR_EMAIL': os.environ.get('GIT_AUTHOR_EMAIL', 'zateya@localhost'),
            'GIT_COMMITTER_NAME': os.environ.get('GIT_COMMITTER_NAME', 'Затея'),
            'GIT_COMMITTER_EMAIL': os.environ.get('GIT_COMMITTER_EMAIL', 'zateya@localhost'),
